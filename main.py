...
def download_architecture_contents_file(input_arch):
    # Hard coding the debian mirror url as provided
    # Please modify according to the need
    mirror_url = "http://ftp.uk.debian.org/debian/dists/stable/main/"
    architecture_contents_url = f"{mirror_url}/Contents-{input_arch}.gz"
    ...
    try:
        ...
        with open(f"Contents-{input_arch}.gz", "wb") as f:
...
def parse_arch_contents_file(file_path):
    package_files_count_check = defaultdict(int)

    try:
        # parsing the contents of a gzipped file line by line and
        # counting occurrences of package names
...
def display_architecture_top_packages(package_files_count_check):
    sorted_packages = sorted(
        package_files_count_check.items(), key=itemgetter(1), reverse=True
    )
